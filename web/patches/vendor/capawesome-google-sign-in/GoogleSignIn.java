...
public class GoogleSignIn {
    ...
    public void signIn(@NonNull SignInOptions options, @NonNull NonEmptyResultCallback<SignInResult> callback) {
        ...
        final GetCredentialRequest request = new GetCredentialRequest.Builder().addCredentialOption(signInOption).build();

        /*
         * Capacitor may invoke plugin methods off the Android main thread. CredentialManager + SIWG
         * must run on the main looper — otherwise the flow often completes immediately as "canceled".
         */
        final Activity activity = plugin.getActivity();
        if (activity == null) {
            callback.error(new Exception("Google Sign-In requires an active Activity."));
            return;
        }
