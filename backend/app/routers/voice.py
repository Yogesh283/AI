...
async def _build_realtime_instructions(
    *,
    ...
    return (
        ...
        "Truth rule for live stats: treat scores, ranks, and prices as certain only when the live web data message "
        "actually contains them; wrong confident numbers are unacceptable. "
        "Never read aloud a full long ranking list unless that order appears in the live message—summarize from snippets. "
        "General knowledge: always answer definitions, how-things-work, advice, and history from training confidently "
        "unless the system note says live lookup failed—in that case still help with stable facts and clearly separate "
        "what needs a fresh web check. "
        "If snippets are missing or lookup failed, give a full helpful spoken answer anyway; only hedge exact "
        "today-specific figures (rates, live match scores, breaking headlines) you cannot verify—never refuse the "
        "whole question with 'I don't know'.\n"
